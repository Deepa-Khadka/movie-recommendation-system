...
export const getHighestRatedMoviesAction = () => async (dispatch) => {
  try {
    dispatch({ type: moviesConstants.MOVIE_HIGHEST_RATED_REQUEST });
    const response = await getHighestRatedMoviesService();
    dispatch({
      type: moviesConstants.MOVIE_HIGHEST_RATED_SUCCESS,
      payload: response.data,
    });
  } catch (error) {
    dispatch({
      type: moviesConstants.MOVIE_HIGHEST_RATED_FAIL,
      payload: error.message,
    });
  }
};
