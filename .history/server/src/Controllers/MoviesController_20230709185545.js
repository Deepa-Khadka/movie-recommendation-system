...
const getMovies = asyncHandler(async (req, res) => {
  try {
    ...
    const movies = await Movie.find(query).sort({ createdAt: -1 }).skip(skip);
...
const getRecommendataion = async (req, res) => {
  ...
  const user = await User.find({_id: targetUid})
