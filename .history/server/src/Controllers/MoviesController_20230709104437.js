...
const getRecommendataion = async(req, res) => {
  ...
  users.forEach(user => {
    ...
    movies.forEach(movie => {
      const watched = movie.reviews.some(review =>  mongoose.Types.ObjectId( review.userId) === user._id)
      if(watched) {
  ...
  console.log("targetUserWatchedList",targetUserWatchedList)
