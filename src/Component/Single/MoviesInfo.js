    ...
    alt={movie.name}  className='w-full h-full object-cover'/>
     ...
     <div className='grid sm:grid-cols-5 grid-cols-3 gap-4 p-6 bg-main border border-gray-800 rounded-lg '>
      ...
      <div className='col-span-1 flex-col border-r content-center justify-center border-border'>
        <button className='w-10 h-10 flex justify-center rounded-lg bg-white items-center bg-opacity-20'>
          <FaShareAlt/>
        </button>
      ...
      <div className='sm:col-span-2 col-span-3 flex justify-end font-medium text-sm '>
       <Link to={`/watch/${movie?.name}`} className='bg-dry hover:bg-subMain transition border-2 border-subMain rounded-full flex-rows  gap-4 w-full sm:py-3'>
